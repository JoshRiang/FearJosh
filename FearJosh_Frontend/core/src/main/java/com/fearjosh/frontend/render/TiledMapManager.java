...
public class TiledMapManager implements Disposable {
    ...
    // Layers that need Y-sorted rendering (furniture/objects that player can walk behind/in front of)
    ...
    private void logDoorInfo() {
        ...
        for (MapLayer layer : currentMap.getLayers()) {
            Gdx.app.log("TiledMapManager", "  - " + layer.getName() + " (type: " + layer.getClass().getSimpleName() + ")");
        }
    }
    ...
    public void renderBelowPlayer(OrthographicCamera camera) {
        if (mapRenderer == null || currentMap == null) return;
    ...
    public void renderAbovePlayer(OrthographicCamera camera) {
        ...
        // Find the "Player" layer index and render everything after it, excluding Y-sorted layers
        int playerLayerIndex = getLayerIndex("Player");
     ...
     * @param batch SpriteBatch for rendering (must already be begun with camera projection)
     * @param playerY Player's Y position (bottom of player sprite)
     * @param playerRenderer Callback to render the player
     */
    public void renderWithYSort(OrthographicCamera camera, SpriteBatch batch, float playerY, Runnable playerRenderer) {
        ...
        // Add player to the sorted list
        elements.add(new YSortedElement(playerY, playerRenderer));
    ...
    /**
     * Render layers below player, EXCLUDING Y-sorted layers (which are rendered separately)
     */
     ...
     * @param fromRoom The room the player is coming from (e.g., "hallway", "HALLWAY")
     * @return SpawnInfo with x,y coordinates, or null if not found
     */
    public SpawnInfo getSpawnPoint(String fromRoom) {
        if (currentMap == null || fromRoom == null) return null;
        ...
        for (MapObject obj : objects) {
            ...
            if (objFromRoom != null) {
                ...
                if (matches) {
                    ...
                    Gdx.app.log("TiledMapManager", "Found spawn point for from_room='" + fromRoom + "' (matched '" + objFromRoom + "') at (" + x + ", " + y + ")");
                    return new SpawnInfo(x, y, fromRoom);
                }
            }
        }
        
        Gdx.app.log("TiledMapManager", "No spawn point found for from_room='" + fromRoom + "'. Available spawn points:");
        for (MapObject obj : objects) {
            String objFromRoom = obj.getProperties().get("from_room", String.class);
     ...
     * Looks for a spawn object with from_room="first_spawn" or "start" or "initial".
     */
    ...
    public Array<DoorInfo> getDoors() {
        ...
        for (MapObject obj : objects) {
            if (obj instanceof RectangleMapObject) {
                ...
                if (doorName == null) doorName = "Door";
    ...
    public RoomId checkDoorTransition(float x, float y, float width, float height) {
        ...
        for (DoorInfo door : doors) {
            if (playerBounds.overlaps(door.bounds)) {
                ...
                Gdx.app.log("TiledMapManager", "Door overlap! " + door.doorName + " -> " + door.targetRoom + " (parsed: " + targetRoom + ")");
                if (targetRoom != null) {
                    return targetRoom;
                }
            }
    ...
    private RoomId parseRoomName(String roomName) {
        ...
        switch (normalized) {
            case "CLASSROOM_1A": return RoomId.CLASS_1A;
            case "CLASSROOM_2A": return RoomId.CLASS_2A;
            case "CLASSROOM_3A": return RoomId.CLASS_3A;
            case "CLASSROOM_4A": return RoomId.CLASS_4A;
            case "CLASSROOM_5A": return RoomId.CLASS_5A;
            case "CLASSROOM_6A": return RoomId.CLASS_6A;
            case "CLASSROOM_7A": return RoomId.CLASS_7A;
            case "CLASSROOM_8A": return RoomId.CLASS_8A;
            case "CLASSROOM_1B": return RoomId.CLASS_1B;
            case "CLASSROOM_2B": return RoomId.CLASS_2B;
            case "CLASSROOM_3B": return RoomId.CLASS_3B;
            case "CLASSROOM_4B": return RoomId.CLASS_4B;
            case "CLASSROOM_5B": return RoomId.CLASS_5B;
            case "CLASSROOM_6B": return RoomId.CLASS_6B;
            case "CLASSROOM_7B": return RoomId.CLASS_7B;
            case "CLASSROOM_8B": return RoomId.CLASS_8B;
            case "JANITOR": return RoomId.JANITOR;
            case "RESTROOM": return RoomId.RESTROOM;
            case "TEACHERS_ROOM": return RoomId.TEACHERS_ROOM;
            case "GYM": 
            case "GYMNASIUM": return RoomId.GYM;
            case "HALLWAY": 
            case "MAIN_HALLWAY": return RoomId.HALLWAY;
            case "LOBBY": 
            case "MAIN_LOBBY": return RoomId.LOBBY;
            case "PARKING": 
            case "PARKING_LOT": return RoomId.PARKING;
            default:
                // Try direct match
                try {
     ...
     * Use this to create lockers, chests, switches, etc. that change tile appearance.
     */
    ...
    /**
     * Load tile interactables by scanning all tile layers for tiles with "interactable" property.
    ...
    private void loadTileInteractables() {
        ...
        for (MapLayer mapLayer : currentMap.getLayers()) {
            ...
            for (int y = 0; y < tileLayer.getHeight(); y++) {
                for (int x = 0; x < tileLayer.getWidth(); x++) {
                    ...
                    if (isInteractable == null || !isInteractable) continue;
        ...
        Gdx.app.log("TiledMapManager", "Loaded " + tileInteractables.size() + " tile interactables from tileset properties");
    }
    ...
    public boolean toggleTileInteractable(TileInteractable interactable) {
        if (currentMap == null || interactable == null) return false;
    ...
    public boolean isAreaWalkableDetailed(float x, float y, float width, float height) {
        ...
        for (float checkX = x; checkX <= x + width; checkX += step) {
            if (!isWalkable(checkX, y)) return false;
        }
        
        // Check top edge
        for (float checkX = x; checkX <= x + width; checkX += step) {
            if (!isWalkable(checkX, y + height)) return false;
        }
        
        // Check left edge
        for (float checkY = y; checkY <= y + height; checkY += step) {
            if (!isWalkable(x, checkY)) return false;
        }
        
        // Check right edge
        for (float checkY = y; checkY <= y + height; checkY += step) {
            if (!isWalkable(x + width, checkY)) return false;
        }
    ...
    public Array<Rectangle> getCollidingTiles(float x, float y, float width, float height) {
        ...
        if (currentMap == null) return tiles;
        ...
        TiledMapTileLayer floorLayer = (TiledMapTileLayer) currentMap.getLayers().get(FLOOR_LAYER);
        if (floorLayer == null) return tiles;
    ...
    public Rectangle getWalkableBounds() {
        ...
        TiledMapTileLayer floorLayer = (TiledMapTileLayer) currentMap.getLayers().get(FLOOR_LAYER);
        if (floorLayer == null) return null;
        ...
        for (int y = 0; y < mapHeightTiles; y++) {
            for (int x = 0; x < mapWidthTiles; x++) {
                ...
                if (cell != null && cell.getTile() != null) {
                    ...
                    Boolean walkable = cell.getTile().getProperties().get("walkable", Boolean.class);
                    boolean isWalkable = (walkable != null) ? walkable : WALKABLE_TILE_IDS.contains(cell.getTile().getId());
